...
// Client and Credential interfaces (matching the dashboard)
interface Client {
  id: string;
  clientName: string;
...
export class PDFExporter {
  ...
  constructor(config: PDFConfig = defaultPDFConfig) {
    this.config = config;
    this.doc = new jsPDF();
    this.setupDocument();
  }
  ...
  async exportClientCredentials(
    ...
    // Generate filename
    const filename = `credentials_${client.clientName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
  ...
  private addCredentialsTable(
    ...
    credentials.forEach((credential, index) => {
      ...
      const notes = credential.notes.length > 30 
        ? credential.notes.substring(0, 30) + '...' 
        : credential.notes;
      this.doc.text(notes || 'N/A', colPositions[3], yPosition + 2);
...
/**
 * Utility function to export client credentials
 */
export async function exportClientCredentialsToPDF(
  client: Client,
  credentials: Credential[],
  decryptPassword: (encryptedPassword: string) => string,
  config?: PDFConfig
  ...
  await exporter.exportClientCredentials(client, credentials, decryptPassword);
}

/**
 * Utility function to generate a preview of the PDF configuration
 */
export function generatePDFPreview(config: PDFConfig): string {
  return `
